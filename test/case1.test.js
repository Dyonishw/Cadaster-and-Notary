...
contract('CadasterDatabase', function (accounts) {
    ...
    it('should finish transaction succesfully', async function () {
      ...
      await expectThrow(notar.setInitialParameters(buyerAddress,741741, 963963, {from: externalAddress}));
      ...
      await expectThrow(notar.agreementForTransaction({from: externalAddress}));
      await notar.agreementForTransaction({from: firstLandlord});
      await notar.agreementForTransaction({from: buyerAddress});

      await expectThrow(notar.withdrawBalances({from: buyerAddress}));
      ...
      console.log(noulproprietar +" this is the new owner (buyer)");
